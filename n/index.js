...
router.post("/", async (req, res) => {
    ...
    if (text == '') {
        ...
        response = `CON Welcome to Gateway.
...
//  2. WITHDRAW FUNDS
        ...
        response = `END You have withd
        rawn KES: `+data[1]+` from account: `+phoneNumber.substring(1);        
    }
...
async function addUserDataToDB(userId){    
    let docRef = firestore.collection('accounts').set(userId);
  ...
  async function getSenderDetails(senderId){
    return new Promise(resolve => {
       let docRef = firestore.collection('accounts').set(senderId);
       docRef.get().then((doc) => {
        resolve ({   
...
function getUserAddressUrl(userAddress){
  ...
  async function getReceiverDetails(recipientId){
    return new Promise(resolve => {
       let docRef = firestore.collection('accounts').set(recipientId);
       docRef.get().then((doc) => { 
        resolve ({   
